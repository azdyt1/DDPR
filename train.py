...
def read_train_file(file_path_train):
    ...
    with open(file_path_train, 'r') as f:
        ...
        for line in tqdm(f, total=3000000):
            ...
            cate_id = int(line_split[3])
            his_item_seq = line_split[4]
            # matching_seq_ = line_split[5]
            label = float(line_split[5])
            weight = float(line_split[6])

            if his_item_seq == '' or his_item_seq == 'NULL':
                his_item_seq_padding = [0] * 25
                # his_cate_seq_padding = [0] * 25
                # his_mathcing_item_seq_padding = [0] * 200
                # his_mathcing_cate_seq_padding = [0] * 200
            else:
                his_item_seq_padding = [int(x) for x in his_item_seq.split('-')] + [0] * 25
                his_item_seq_padding = his_item_seq_padding[:25]

                # his_cate_seq_padding = [sku_info_map[x]['cate_id'] for x in his_item_seq_padding]

                # his_mathcing_item_seq_padding = [int(x) for x in matching_seq_.split('-')] + [0] * 200
                # his_mathcing_item_seq_padding = his_mathcing_item_seq_padding[:200]

                # his_mathcing_cate_seq_padding = [sku_info_map[x]['cate_id'] for x in his_mathcing_item_seq_padding]


            item_list.append(item_id)
            cate_list.append(cate_id)
            his_item_seq_list.append(his_item_seq_padding)
            # his_cate_seq_list.append(his_cate_seq_padding)
            # match_item_seq_list.append(his_mathcing_item_seq_padding)
            # match_cate_seq_list.append(his_mathcing_cate_seq_padding)
...
def read_valid_file(file_path_train):
    ...
    with open(file_path_train, 'r') as f:
        ...
        for line in tqdm(f, total=200000):
            ...
            #     break
            line_split = line.strip('\n').split('\t')
            queryid = line_split[1]
            uid = int(line_split[1])
            item_id = int(line_split[2])
            cate_id = int(line_split[3])
            his_item_seq = line_split[4]
            # match_item_seq_ = line_split[5]
            label = float(line_split[5])
            weight = float(line_split[6])
            ...
            if his_item_seq == '' or his_item_seq == 'NULL':
                ...
                # his_mathcing_item_seq_padding = [int(x) for x in match_item_seq_.split('-')] + [0] * 200
                # his_mathcing_item_seq_padding = his_mathcing_item_seq_padding[:200]

                # his_mathcing_cate_seq_padding = [sku_info_map[x]['cate_id'] for x in his_mathcing_item_seq_padding]

            item_valid_list.append(item_id)
            cate_valid_list.append(cate_id)
            his_item_seq_valid_list.append(his_item_seq_padding)
            # his_cate_seq_test_list.append(his_cate_seq_padding)
            # match_item_seq_test_list.append(his_mathcing_item_seq_padding)
            # match_cate_seq_test_list.append(his_mathcing_cate_seq_padding)
            label_valid_list.append(label)
            weight_valid_list.append(weight)
...
def read_test_file(file_path_test):
    ...
    with open(file_path_test, 'r') as f:
        ...
        for line in tqdm(f, total=200000):
            ...
            cate_test_list.append(cate_id)
            his_item_seq_test_list.append(his_item_seq_padding)
            # his_cate_seq_test_list.append(his_cate_seq_padding)
            # match_item_seq_test_list.append(his_mathcing_item_seq_padding)
            # match_cate_seq_test_list.append(his_mathcing_cate_seq_padding)
            label_test_list.append(label)
            weight_test_list.append(weight)
...
def generate_train_batch(batch_data_index):
    ...
    cate = np.reshape([cate_list[i] for i in batch_data_index], [-1,1])
    his_item_seq = [his_item_seq_list[i] for i in batch_data_index]
    his_cate_seq = []
    for l in his_item_seq:
        his_cate_seq.append([sku_info_map[i]['cate_id'] for i in l])
    match_item_seq = [user_train_matching_seq_map[query_list[i]] for i in batch_data_index]
    match_cate_seq = []
    for l in match_item_seq:
        match_cate_seq.append([sku_info_map[i]['cate_id'] for i in l])
    his_long_item_seq = [user_train_long_seq_map[query_list[i]] for i in batch_data_index]
    his_long_cate_seq = []
    for l in his_long_item_seq:
        his_long_cate_seq.append([sku_info_map[i]['cate_id'] for i in l])
    label = np.reshape([label_list[i] for i in batch_data_index], [-1,1])
    weight = np.reshape([weight_list[i] for i in batch_data_index], [-1,1])
    user = np.reshape([user_list[i] for i in batch_data_index], [-1,1])

    return [item, cate, his_item_seq, his_cate_seq, match_item_seq, match_cate_seq, label, weight, user, his_long_item_seq, his_long_cate_seq]


def generate_valid_batch(batch_size, global_index):
    item = np.reshape(item_valid_list[global_index-batch_size:global_index], [-1, 1])
    cate = np.reshape(cate_valid_list[global_index-batch_size:global_index], [-1, 1])
    his_item_seq = his_item_seq_valid_list[global_index-batch_size:global_index]
    his_cate_seq = []
    for l in his_item_seq:
        his_cate_seq.append([sku_info_map[i]['cate_id'] for i in l])
    match_item_seq = [user_valid_matching_seq_map[u] for u in query_valid_list[global_index-batch_size:global_index]]
    match_cate_seq = []
    for l in match_item_seq:
        match_cate_seq.append([sku_info_map[i]['cate_id'] for i in l])
    his_long_item_seq = [user_valid_long_seq_map[u] for u in query_valid_list[global_index - batch_size:global_index]]
    his_long_cate_seq = []
    for l in his_long_item_seq:
        his_long_cate_seq.append([sku_info_map[i]['cate_id'] for i in l])
    label = np.reshape(label_valid_list[global_index-batch_size:global_index], [-1, 1])
    weight = np.reshape(weight_valid_list[global_index-batch_size:global_index], [-1, 1])
...
def generate_test_batch(batch_size, global_index):
    ...
    cate = np.reshape(cate_test_list[global_index-batch_size:global_index], [-1, 1])
    his_item_seq = his_item_seq_test_list[global_index-batch_size:global_index]
    his_cate_seq = []
    for l in his_item_seq:
        his_cate_seq.append([sku_info_map[i]['cate_id'] for i in l])
    match_item_seq = [user_test_matching_seq_map[u] for u in query_test_list[global_index-batch_size:global_index]]
    match_cate_seq = []
    for l in match_item_seq:
        match_cate_seq.append([sku_info_map[i]['cate_id'] for i in l])
    his_long_item_seq = [user_test_long_seq_map[u] for u in query_test_list[global_index - batch_size:global_index]]
    his_long_cate_seq = []
    for l in his_long_item_seq:
        his_long_cate_seq.append([sku_info_map[i]['cate_id'] for i in l])
    label = np.reshape(label_test_list[global_index-batch_size:global_index], [-1, 1])
    weight = np.reshape(weight_test_list[global_index-batch_size:global_index], [-1, 1])
...
def train(sku_info_map, exp_name, model_name, sku_novelty_map):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        trace_mmr, trace_dpp, trace_cate, trace_pmf, trace_ssd, trace_rl, trace_ddpr = rerank_trace(trace,
        ...
        auc_rl, ndcg_rl, coverage_rl, ild_rl, div_rl, entropy_rl = eval_full_data(trace_rl, sku_info_map, sku_emb,
                                                                                  're-rank')
        auc_ddpr, ndcg_ddpr, coverage_ddpr, ild_ddpr, div_ddpr, entropy_ddpr = eval_full_data(trace_ddpr, sku_info_map, sku_emb,
                                                                                  're-rank')
        ...
        file_log.write(
            'auc_ddpr: ' + str(auc_ddpr) + ', ndcg_ddpr: ' + str(ndcg_ddpr) + ', coverage_ddpr: ' + str(
                coverage_ddpr) + ', ild_ddpr: ' + str(
                ild_ddpr) + ', div_ddpr: ' + str(div_ddpr) + ', entropy_ddpr: ' + str(entropy_ddpr) + '\n')
        ...
        print('auc_rl:{}, ndcg_rl:{}, coverage_rl:{}, ild_rl:{}, div_rl:{}, entropy_rl{}'.format(auc_rl, ndcg_rl,
                                                                                                 coverage_rl, ild_rl,
                                                                                                 div_rl, entropy_rl))
        print('auc_ddpr:{}, ndcg_ddpr:{}, coverage_ddpr:{}, ild_ddpr:{}, div_ddpr:{}, entropy_ddpr{}'.format(auc_ddpr, ndcg_ddpr,
                                                                                                 coverage_ddpr, ild_ddpr,
                                                                                                 div_ddpr, entropy_ddpr))

def DDPR(sku_list, sku_info_map, dtb, lambdaConstant=0.10, betaConstant=0.001 , df=0.5, topN=20):
    s, r = [], sku_list.copy()
    r = sorted(r, key=lambda item: -item['prediction'])
    new_score = []
    len_r = len(r)
    ...
    while len(s) < len_r:
        ...
        second = 0.0
        if len(s) > 1:
            s_dtb_temp = s_dtb.copy()
            score_mp = {}
            cnt_map = {}
            total_origin = 0
            for cate in s_dtb_temp.keys():
                cnt_map[cate] = {}
                pro_not = (s_dtb_temp[cate]) / (len(s) + 1)
                cnt_map[cate]['not'] = pro_not * math.log(pro_not)
                pro_add = (s_dtb_temp[cate]+1) / (len(s) + 1)
                cnt_map[cate]['add'] = pro_add * math.log(pro_add)
                total_origin += pro_not

            for cate in s_dtb_temp.keys():
                score_mp[cate] = total_origin - cnt_map[cate]['not'] + cnt_map[cate]['add']
            score_mp['new'] = (1 / (len(s) + 1))*math.log((1 / (len(s) + 1))) + total_origin
        for i in range(len(r)):
            item_dict = r[i]
            firstPart = item_dict['prediction']
            cate3 = sku_info_map[item_dict['skuid']]['cate_id']
            # third
            thirdPart = 0.0
            if len(s) > 1:
                if cate3 not in s_dtb_temp.keys():
                    thirdPart = score_mp['new']
                else:
                    thirdPart = score_mp[cate3]
            equationScore = firstPart + (-1.0 * lambdaConstant * thirdPart)
            if equationScore > score:
                score = equationScore
                selectOne = i
                select_cate3 = cate3
        if selectOne == None:
            selectOne = i
            select_cate3 = sku_info_map[r[i]['skuid']]['cate_id']
        s.append(r[selectOne])
        if select_cate3 not in s_dtb.keys():
            s_dtb[select_cate3] = 0
        s_dtb[select_cate3] += 1
        r.pop(selectOne)
    return s, new_score
...
def test(best_model_path, sku_info_map, model_name, sku_novelty_map):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        auc_rl, ndcg_rl, coverage_rl, ild_rl, div_rl, entropy_rl = eval_full_data(trace_rl, sku_info_map, sku_emb,
                                                                                  're-rank')
        auc_ddpr, ndcg_ddpr, coverage_ddpr, ild_ddpr, div_ddpr, entropy_ddpr = eval_full_data(trace_ddpr, sku_info_map,
                                                                                              sku_emb,
                                                                                              're-rank')
        ...
        print('auc_rl:{}, ndcg_rl:{}, coverage_rl:{}, ild_rl:{}, div_rl:{}, entropy_rl{}'.format(auc_rl, ndcg_rl,
                                                                                                 coverage_rl, ild_rl,
                                                                                                 div_rl, entropy_rl))
        print('auc_ddpr:{}, ndcg_ddpr:{}, coverage_ddpr:{}, ild_ddpr:{}, div_ddpr:{}, entropy_ddpr{}'.format(auc_ddpr,
                                                                                                             ndcg_ddpr,
                                                                                                             coverage_ddpr,
                                                                                                             ild_ddpr,
                                                                                                             div_ddpr,
...
def rerank_trace(trace, topk, lambdaConstantMMR=0.9, lambdaConstantDPP=0.9, lambdaConstantCATE=0.01, lambdaConstantPMFA=0.5, lambdaConstantPMFB=0.5, lambdaConstantSSD=0.5, sku_emb=None, sku_info_map=None, sku_novelty_map=None):
    ...
    for trace_, sku_list in tqdm(trace.items()):
        ...
        sku_sorted_list_ddpr, new_score = DDPR(sku_list_s_k.copy(), sku_emb)
        ...
        sku_sorted_list_ddpr.extend(sku_list_s_k_)
        ...
        trace_ddpr[trace_] = sku_sorted_list_ddpr

    return trace_mmr, trace_dpp, trace_cate, trace_pmf, trace_ssd, trace_rl, trace_ddpr
