...
class DRN(object):
    ...
    def __init__(self, input_format):
        ...
        self.cateid = tf.placeholder(tf.int32, shape=[None, None])

        self.hisids = tf.placeholder(tf.int32, shape=[None, None])
        self.hiscateids = tf.placeholder(tf.int32, shape=[None, None])

        self.divids = tf.placeholder(tf.int32, shape=[None, None])
        self.divcateids = tf.placeholder(tf.int32, shape=[None, None])

        self.label = tf.placeholder(tf.float32, shape=[None, None])
        self.weight = tf.placeholder(tf.float32, shape=[None, None])
        ...
        self.target_cate_in = tf.reshape(tf.nn.embedding_lookup(self.emb_cate, self.cateid), shape=[-1, self.params['dims']])
        # self.target = tf.concat([self.target_in, self.target_cate_in], -1)
        self.target_join_in = tf.concat([self.target_item_in, self.target_cate_in], -1)

        # user interaction encoding
        self.seq_his_item_in = tf.nn.embedding_lookup(self.emb_sku, self.hisids)
        self.seq_his_cate_in = tf.nn.embedding_lookup(self.emb_cate, self.hiscateids)
        # self.seq_his_30d = tf.concat([self.seq_his_30d_in, self.seq_his_30d_cate_in], -1)
        self.seq_his_join_in = tf.concat([self.seq_his_item_in, self.seq_his_cate_in], -1)
        self.seq_his_join_sum = tf.reduce_sum(self.seq_his_join_in, 1)
        self.seq_his_join_attn = self.get_sequence_embedding_with_transformer('his_short_seq', self.hisids, T=self.params['his_short_seq_max_len'], target_embedding=self.target_join_in,mode='attention', seq_embedding=self.seq_his_join_in, user_embedding=None)

        # predicting layer
        self.deep_out = tf.concat([self.user_emb, self.target_join_in, self.seq_his_join_attn, self.seq_his_join_sum, self.target_join_in*self.seq_his_join_sum], 1)
        ...
        self.index = tf.nn.top_k(tf.squeeze(self.final_layer_out), self.top_k)
    ...
    def train(self, features, sess, data_type):
        ...
        loss, train_op = sess.run([self.loss, self.train_op],
                                  feed_dict={self.skuid: features[0], self.hisids: features[2],
                                             self.divids: features[4], self.label: features[6],
                                             self.weight: features[7],
                                             self.cateid: features[1],
                                             self.hiscateids: features[3],
                                             self.divcateids: features[5], self.uid: features[8]})

        return loss, train_op

    def test(self, features, sess, data_type):

        return sess.run([tf.nn.sigmoid(self.final_layer_out)], feed_dict={self.skuid: features[0], self.hisids: features[2],
                                                 self.divids: features[4], self.label: features[6],
                                                 self.weight: features[7],
                                                 self.cateid: features[1],
                                                 self.hiscateids: features[3],
