...
class Model(object):
    ...
    def output_item_emb(self, sess, inp):
        item_embs = sess.run(self.item_eb, feed_dict={
            self.mid_batch_ph: inp}
        )
        return item_embs
    ...
    def output_predict(self, sess, hist_item_list, hist_mask_list, item_id_list, batch_size):
        user_emb = self.output_user(sess, [hist_item_list, hist_mask_list])
        item_emb = self.output_item_emb(sess, item_id_list)
        self.predict = tf.nn.sigmoid(tf.reduce_max(
            tf.reshape(tf.matmul(user_emb, tf.transpose(tf.expand_dims(item_emb, 1), [0, 2, 1])), [batch_size, -1]),
            -1))
        return self.predict
