...
from data_iterator2 import DataIterator2
from data_iterator3 import DataIterator3
from model import *
from tensorboardX import SummaryWriter

parser = argparse.ArgumentParser()
parser.add_argument('-p', type=str, default='test_generate_matching_list', help='train | test')
parser.add_argument('--dataset', type=str, default='taobao', help='book | taobao')
parser.add_argument('--random_seed', type=int, default=19)
parser.add_argument('--embedding_dim', type=int, default=64)
parser.add_argument('--hidden_size', type=int, default=64)
parser.add_argument('--num_interest', type=int, default=1)
...
def generate_matching_list(sess, test_data, model):
    item_embs = model.output_item(sess)
    ...
    topN = 500
    file_out = open('taobao_valid_data_10000user_matching_list_500.txt', 'w')
    # file_out.write('uuid\tmatching_list\tmatching_list_score\n')
    file_out.write('queryid\tmatching_list\n')
    for uid, hist_item, hist_mask in tqdm(test_data, total=1000//100):
        ...
        #     matching_list = list(np.reshape(I[i * ni:(i + 1) * ni], -1))
        #     matching_list_score = list(np.reshape(D[i*ni:(i+1)*ni], -1))
        #     # file_out.write(str(uid[i])+'\t'+'-'.join([str(x) for x in matching_list])+'\t'+'-'.join([str(y) for y in matching_list_score])+'\n')
        #     file_out.write(str(uid[i])+'\t'+'-'.join([str(x) for x in matching_list])+'\n')
        for i in range(len(uid)):
            matching_list = list(np.reshape(I[i], -1))
            matching_list_score = list(np.reshape(D[i], -1))
            # file_out.write(str(uid[i])+'\t'+'-'.join([str(x) for x in matching_list])+'\t'+'-'.join([str(y) for y in matching_list_score])+'\n')
            file_out.write(str(uid[i])+'\t'+'-'.join([str(x) for x in matching_list])+'\n')
...
def test_rank(
    ...
    file_out = open('taobao_validdata_10000user_offline_matching_predict.txt', 'w')
    # file_out.write('uuid\titem_id\tprediction\tlabel\n')

    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        model.restore(sess, best_model_path)

        test_file = 'taobao_validdata_10000user_offline.txt'
        batch_size = 10000
        test_data = DataIterator2(test_file, batch_size, maxlen, train_flag=2)
        print('begin test batch:')
        for user_id_list, item_id_list, hist_item_list, hist_mask_list, label in tqdm(test_data, total=200000//10000):
            user_emb = model.output_user(sess, [hist_item_list, hist_mask_list])
            item_emb = model.output_item_emb(sess, item_id_list)

            predict = tf.reduce_max(tf.reshape(tf.matmul(user_emb, tf.transpose(tf.expand_dims(item_emb,1), [0,2,1])), [batch_size, -1]), -1)
            predict = sess.run(predict)
            # predict = model.output_predict(sess, hist_item_list, hist_mask_list, item_id_list, batch_size)
            for i in range(len(user_id_list)):
                file_out.write(str(user_id_list[i])+'\t'+str(item_id_list[i])+'\t'+str(predict[i])+'\t'+str(label[i])+'\n')

def train_generate_matching_list(
        test_file,
        cate_file,
        item_count,
        dataset = "book",
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        test_file = 'taobao_traindata_forcomirec_2.txt'
        batch_size = 1000
        test_data = DataIterator3(test_file, batch_size, maxlen, train_flag=2)
        print('begin generating matching list:')
        generate_matching_list(sess, test_data, model)

def test_generate_matching_list(
        test_file,
        cate_file,
        item_count,
        dataset = "book",
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        test_file = 'taobao_validdata_10000user_formatch.txt'
        batch_size = 100
        test_data = DataIterator3(test_file, batch_size, maxlen, train_flag=2)
        print('begin generating matching list:')
        generate_matching_list(sess, test_data, model)
...
if __name__ == '__main__':
    ...
    elif args.p == 'test_rank':
        test_rank(test_file=test_file, cate_file=cate_file, item_count=item_count, dataset=dataset, batch_size=batch_size,
             maxlen=maxlen, model_type=args.model_type, lr=args.learning_rate)
    elif args.p == 'train_generate_matching_list':
        train_generate_matching_list(test_file=test_file, cate_file=cate_file, item_count=item_count, dataset=dataset,
                  batch_size=batch_size,
                  maxlen=maxlen, model_type=args.model_type, lr=args.learning_rate)
    elif args.p == 'test_generate_matching_list':
        test_generate_matching_list(test_file=test_file, cate_file=cate_file, item_count=item_count, dataset=dataset,
                  batch_size=batch_size,
                  maxlen=maxlen, model_type=args.model_type, lr=args.learning_rate)
    else:
        print('do nothing...')
